...
JUDGE_SYSTEM_INSTRUCTION = (
    ...
    "3. context_relevance: did retrieval find the right place to look, not whether the answer "
    "happens to exist there. Judge the retrieval, not the generated answer - a correct 'the "
    "documents don't say' answer is NOT itself evidence of bad retrieval, since that same honest "
    "decline can follow from either great or bad retrieval. Evaluate the chunks on their own, "
    "ignoring how the answer chose to respond. Ask yourself: does the specific fact the question "
    "wants plausibly belong inside the same document, section, or subject area these chunks came "
    "from? If yes, but the chunks just don't happen to state that one detail, that is correct "
    "retrieval of the right material - score it high (4-5). Only score low (1-2) when the chunks "
    "discuss a genuinely different subject than the question is about (wrong document, wrong "
    "entity, wrong topic entirely) - not merely because one specific detail is missing from "
    "otherwise on-topic material.\n"
    "Worked examples: question 'is Python the only language with an interpreter?' with chunks "
    "entirely about Python's own interpreter/tooling - the chunks are squarely on-topic (they're "
    "about interpreters, about Python internals), they simply don't compare against other "
    "languages, so this scores 4-5, not low, even though the answer has to decline. Contrast: "
    "question 'what does the TCP document say about medicines?' with chunks about TCP networking "
    "- genuinely wrong-subject material for what the question is asking about, so this scores "
    "low. 5 = retrieval found exactly the right material to look in, whether or not that material "
    "contains the answer.\n\n"
